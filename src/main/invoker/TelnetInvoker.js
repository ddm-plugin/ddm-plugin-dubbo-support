...
class TelnetInvoker {
    ...
    invokeMethod( provder, methodInfo, code) {
        ...
        return new Promise((resolve, reject) => {
            ...
            socket.on('error', function(error) {
                resolve({
                    success : false,
                    data :  app.pluginT("dubbo.invokePage.connectProviderError", {e : error}),
                    elapsedTime : 0
                });
                socket.end();
                return;
            ...
            socket.on("timeout", () => {
                reject(new Error(app.pluginT("dubbo.invokePage.invokeTimeOut")) )
                socket.end();
            })
    ...
    resolveResponse(result) {
        ...
        if (errorMessage.length == 4) {
            ...
            try {
                return new InvokeUtils.InvokeResult(jsonFormat.format(data), elapsedTime);
            } catch (e) {
                // 日后再收集
            }
