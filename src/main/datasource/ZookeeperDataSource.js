...
class ZookeeperDataSource {
  ...
  async getProviderList(dataSourceInfo, serviceInfo) {
    ...
    for (let i = 0; i < children.length; i++) {
      ...
      // 服务是否被禁用
      const disabledAddresses = await serviceDisabledMap.computeIfAbsent(this.buildConfigurationPath(providerInfo), async () => dubboConfigurationUtils.getDisableAddresses(await this.getJsonConfiguration(dataSourceInfo, serviceInfo, providerInfo)))
  ...
  async invokeMethod(dataSourceInfo, serviceInfo, provder, methodInfo, code, invokerType) {
    return await this.invoker.invokeMethod(provder, methodInfo, code, invokerType);
  }
